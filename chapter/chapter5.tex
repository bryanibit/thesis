...
\centering
\subfigure[]{
\includegraphics[width=2.6in]{chapter5/reconstruction/DJI_0104.JPG}
\label{example-gcp-image}}
\subfigure[]{\includegraphics[width=2.6in]{chapter5/reconstruction/sixbuild.PNG}
...
\begin{figure}[!htbp]
	...
	\subfigure[稀疏点云]{
		...
		\includegraphics[width=2.5in]{chapter5/reconstruction/sparse_perspect1.png}}
	\subfigure[稠密点云]{
...
与上文雾岚山庄数据集处理方法相似，通过79张图片可以得到自动化学院的三维地图结构，图~\ref{fig:sparse1}为稀疏点云在RVIZ中的显示效果，图~\ref{fig:dense1}为稠密点云在Meshlab中的显示效果图。该数据集同样设置地面控制点以衡量构图精度，共有三个地面控制点，分布如图\ref{example-gcp-sixbuild}所示，该图是展示在Google Earth中的效果，可以看出没有设置落在构图边缘的控制点，控制点整体分布均匀。最后通过实验可以得到井队对比结果，如表~\ref{table:gcp_error_six} 所示，各列数据分别表示地面控制点的真实测量值与实验三维重建得到的模型对应点的东向、北向以及绝对误差值。由于三个点没有收到构图边缘变形的影响，所以结果稍好于好于雾岚山庄，得到平均控制点误差为$0.867m$。实际上由于本实验使用图片以及图片EXIF 信息中的经纬高数据，但是实际上EXIF 中的经纬高数据不是很准确，大约误差在$5m$ 左右，如果想要得到结果更高的地图，需要在构图时引入RTK，改善构图的精度。
