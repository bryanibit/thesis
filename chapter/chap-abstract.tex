...
Epipolar constraint is adopted to remove false matches so that the unordered datasets become "orderly". Afterwards, the two images with the largest number of matching feature points are reconstructed to obtain the initial point clouds with the multi-view geometry algorithms. And the rest of the images are incrementally added to obtain the camera pose of all the images and the sparse point cloud of the entire geographic space.
