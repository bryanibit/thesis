...
三维重建可以分为两部分：稀疏点云与相机位姿的获取、点云稠密化。其中稀疏点云与相机位姿的获取使用的是Structure from Motion（SfM），在技术上与当前热门的Visual-SLAM （V-SLAM）比较类似，只是SfM侧重于三维重建，而SLAM侧重于实时定位，并预测下一时刻的位置。SfM可以分为视觉前端与优化后端，前端涉及相机模型、坐标转换、特征提取与匹配以及多视图几何等相关内容，最后得到粗略的相机位姿与3D点坐标，后端涉及优化前端得到的相机位姿（即精炼相机位姿）与3D点坐标、去除误差大的3D点。点云稠密使用的技术为Multi-View Stereo（MVS），MVS的实现方式多种多样，本文采用基于像素区块的深度图计算方法，对每张图片寻找参考图，根据参考图与SfM恢复的相机位姿得到该张图片的深度图，最后融合所有图片的深度图形成稠密点云。
\section{相机模型}
...
将世界坐标系下的3D点表示为齐次坐标$(X_w,Y_w,Z_w,1)$，式\ref{R-t-equation}可以改写为：
\begin{equation}
%\sideset{^a_b}{^c_d}\prod^{n}_{k=1}
...
\left\{
\begin{aligned}
&u^{rdist} = u + (u-u_o)(k_1 r^2+k_2 r^4+ k_3 r^6 + ...) \\
&v^{rdist} = v + (v-v_o)(k_1 r^2+k_2 r^4+ k_3 r^6 + ...) \\
&r^2 = (u-u_o)^2 + (v-v_o)^2 \\
\end{aligned}
\right.
...
%\end{align}
%\end{subequations}
其中，$u^{rdist}$、$v^{rdist}$为矫正径向失真后的图像坐标点，$u$、$v$为含有畸变的图像坐标点，$u_o$、$v_o$为畸变中心，一般为主点坐标，$k_1,k_2,k_3$是未知参数，需要标定才可以得到，而实际校正径向畸变一般也只用到2个或3个参数。
...
\left\{
\begin{aligned}
&u^{tdist} = u + (2p_2 (u-u_o)(v-v_o) + p_1(r^2 + 2(u-u_o)^2))(1+p_3 r^3 + p_4 r^4 + ...) \\
&v^{tdist} = v + (2p_1 (u-u_o)(v-v_o) + p_2(r^2 + 2(v-v_o)^2))(1+p_3 r^3 + p_4 r^4 + ...) \\
&r^2 = (u-u_o)^2 + (v-v_o)^2 \\
\end{aligned}
\right.
