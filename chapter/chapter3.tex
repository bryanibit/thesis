...
\begin{equation}
E=arg\quad\mathop{{min}}\limits_{x} \sum_{j=1}^{k}\left \|f_j(x) \right \|
=\sum_{j}\rho_j(\left \| \pi(P_c,X_k)-x_j \right \|_2^2)
\label{repro-equation}
\end{equation}
为解决该问题，使用最多的方法是Levenberg-Marquardt\cite{Hartley-MultipleView}，该算法是解决非线性最小二乘问题的通用方法，所以选择LM解决BA问题。LM将非线性问题转变为一系列正则线性问题。设$J(x)$是$f(x)$的雅各比矩阵，则
