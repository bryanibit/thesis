...
上式简写为$Ab=0$，其中$A$为$3\times 9$矩阵，$rank(A)=2$，这是因为每队匹配点只提供两个量：$u$和$v$，$b$为$9\times 1$矩阵，由于只需要维持$b$各项的交比不变即可，所以有8个自由度，故需要4对匹配点即可在这个线性系统上获得足够的条件计算得到单应性矩阵$H$的各项。但是如果我们有很多匹配点，这时候
