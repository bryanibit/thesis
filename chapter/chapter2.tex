...
对于每帧IPM俯视图，车身为无效信息且占据图像大量位置，对地图拼接造成干扰，所以在拼接地图前，需要将车身移除。由于全景图变换为IPM投影图时，将IPM俯视图变换到车体坐标系中，所以任何一张IPM俯视图的车辆位置与方位是固定的：车辆后轴中心位于图片横轴的中心，纵轴靠下的$1/3$位置处，航向朝上。如图~\ref{carmaskmask1}中左图所示。据此设计一个掩码（Mask），在拼接过程中将车辆去掉，如图~\ref{carmaskmask1} 中右图所示，仅拼接剩下的像素不为零的部分。
