...
\section{本章小结}
本章介绍了如何通过Ladybug全景相机生成的球面全景图与高精度定位导航组合，构建车道级别地图的过程。根据Ladybug与车体的固定位置关系，将全景图像转换到车体坐标系中，完成逆透视投影的过程，并将每个IPM投影的图像通过车辆的高精度定位信息，旋转、平移拼接为完整的道路地图，最后将地图生成GIS数据库的图层，完成道路拓扑构建与道路属性添加的任务，得到的地图可用于无人驾驶导航与规划等任务。
